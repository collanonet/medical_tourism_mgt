...
class WebAppointmentDetailModel {
  ...
  void submitReservation(WebBookingMedicalRecordRequest request) async {
    try {
      ...
      webBooking.value = AsyncData(data: result);
      webBookings.value = AsyncData(
        data: webBookings.value.data?..add(result),
      );
      patient.value = const AsyncData();
      doctorSelected.value = const AsyncData();
      formGroup.reset();
      webBooking.value = const AsyncData();
      bookingByPatient.value = const AsyncData();
    } catch (e) {
      logger.e(e);
  ...
  Future<void> submitData({bool? isClosed}) async {
    if (webBooking.value.hasData) {
      ...
      var request = WebBookingMedicalRecordRequest(
        ...
        isClosed: isClosed ?? webBooking.value.data?.isClosed,
  ...
  void getReservationAll({
    ...
    try {
      ...
      logger.e(e);
      webBookings.value = AsyncData(error: e, data: []);
    }
  }

  ValueNotifier<AsyncData<WebBookingMedicalRecordResponse>> webBookingSelected =
      ValueNotifier(const AsyncData());

  void selectReservation(WebBookingMedicalRecordResponse reservation) {
