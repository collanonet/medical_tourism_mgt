...
class ProcessChartModel{
  ...
  ValueNotifier<AsyncData<PatientFilterResponse>> filterPatientData = ValueNotifier(const AsyncData());
  ...
  void insertFilterPatientChart(FormGroup formGroup,PatientFilterResponse? data){
  ...
  ValueNotifier<AsyncData<PatientFilterResponse>> submitFilterPatientChartData = ValueNotifier(const AsyncData());
  Future<void> submitFilterPatientChart(FormGroup formGroup) async{
    try{
      ...
      final response = await processChartRepository.postFilterpatientChart(
        PatientFilterRequst(
