...
class FacilityModel{
  ...
  Future<void> fetchData(FormGroup formGroup) async{
    ...
    await fetchDetailDropInFacility(formGroup.control('drop_in_facility') as FormGroup);
  }
  ValueNotifier<AsyncData<bool>> submitData = ValueNotifier(const AsyncData());
  void submit(FormGroup formGroup) async{
    await submitDropInDacility(formGroup.control('drop_in_facility') as FormGroup);
  }

  ValueNotifier<AsyncData<List<DetailFacilityHotelResponse>>> detailFacilityHotelData = ValueNotifier(const AsyncData());
  ...
  void insertFacilityHotel(FormGroup formGroup,List<DetailFacilityHotelResponse>? data){
        ...
        hotel.add(
          FormGroup({
            'Person_in_charge_of_arrangements':
                FormControl<String>(value: element.personInChargeOfArrangements), 
            'Name_of_facility': FormControl<String>(value: element.nameOfFacility),
            'location': FormControl<String>(value: element.location), 
            'Person_in_charge_name': FormControl<String>(value: element.personInChargeName),
            'telephone_number': FormControl<String>(value: element.telephoneNumber), 
            'remarks': FormControl<String>(value: element.remarks), 
            'Foreign_language_staff': FormControl<String>(value: element.foreignLanguageStaff),
          }),
        );
      }
    }
  ...
  // ValueNotifier<AsyncData<List<DetailFacilityHotelResponse>>> submitDetailFacilityData = ValueNotifier(const AsyncData());
  ...
  //         personInChargeOfArrangements: element
  //       );
  //       var result = await processChartRepository.postDetailFacilityHospital(
  //           request.copyWith(
  ...
  ValueNotifier<AsyncData<DetailDropInFacilityResponse>> dropInFacilityData = ValueNotifier(const AsyncData());
  ...
  void insertDropInFacility(FormGroup formGroup,DetailDropInFacilityResponse? data){
    var facility = formGroup.control('facility') as FormArray;
    formGroup.control('Person_in_charge_of_arrangements').value = data?.personInChargeOfArrangements;
      ...
      for(var element in data.facilities!){
        facility.add(
          FormGroup({
              ...
              'Person_in_charge_name': FormControl<String>(value: element.personInChargeName), // 担当者名
              'telephone_number': FormControl<String>(value: element.telephoneNumber), // 電話番号
            })
        );
      }
    }
  ...
  ValueNotifier<AsyncData<DetailDropInFacilityResponse>> submitDropInFacilityData = ValueNotifier(const AsyncData());
  Future<void> submitDropInDacility(FormGroup formGroup) async{
    try{
      ...
      final response = await processChartRepository.postDetailFacilityDropIn(
        DetailDropInFacilityRequest.fromJson(
