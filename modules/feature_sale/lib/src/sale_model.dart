...
class SaleModel {
  ...
  Future<void> fetchData(FormGroup formGroup) async {
    try {
      ...
      await fetchServiceFree(formGroup.control('serviceFee') as FormGroup);
  ...
  Future<void> submitDataInfo(FormGroup formGroup) async {
    try {
      ...
      await submitHeadInfo(formGroup);
      await submitServiceFree(formGroup);
      await submitExpenses(formGroup);
      await submitTotal(formGroup);
  ...
  ValueNotifier<AsyncData<HeadInfoResponse>> saleHeadInfoData =
  ...
  void insertHeadInfo(FormGroup formGroup, HeadInfoResponse? data) {
    formGroup.control('medical_expense_deposit').value =
        data?.medicalExpenseDeposit;
    ...
    formGroup.control('actual_cost_breakdown').value =
        data?.actualCostBreakdown;
    formGroup.control('Refund_amount').value = data?.refundAmount;
  }

  ValueNotifier<AsyncData<HeadInfoResponse>> submitHeadInfoData =
      ValueNotifier(const AsyncData());
  ...
  ValueNotifier<AsyncData<ServiceFeeResponse>> serviceFreeData =
      ValueNotifier(const AsyncData());
  ...
  void insertServiceFree(FormGroup formGroup, ServiceFeeResponse? data) {
    var serviceItem = formGroup.control('serviceItem') as FormArray;
        ...
        FormGroup(
          {
            ...
            'unitPrice': FormControl<int>(value: element.unitPrice),
            'amountOfMoney': FormControl<int>(value: element.amountOfMoney),
            'cost': FormControl<int>(value: element.cost),
            'profit': FormControl<int>(value: element.profit),
            'invoiceNo': FormControl<int>(value: element.invoiceNo),
            'paymentDocumentNo':
                FormControl<int>(value: element.paymentDocumentNo),
          },
        ),
      );
    }
    formGroup.control('amountOfMoneyTotalSale').value =
        data.amountOfMoneyTotalSale;
    formGroup.control('amountOfMoneyTotalTax').value =
        data.amountOfMoneyTotalTax;
    formGroup.control('amountOfMoneyTotalAmount').value =
        data.amountOfMoneyTotalAmount;
    formGroup.control('costSale').value = data.costSale;
    formGroup.control('costTax').value = data.costTax;
    formGroup.control('costAmount').value = data.costAmount;
    formGroup.control('profitSale').value = data.profitSale;
    formGroup.control('profitTax').value = data.profitTax;
    formGroup.control('profitAmount').value = data.profitAmount;
    formGroup.control('tax').value = data.tax;
    formGroup.control('taxExcluded').value = data.taxExcluded;
    formGroup.control('taxExempt').value = data.taxExempt;
  }

  ValueNotifier<AsyncData<ServiceFeeResponse>> submitServiceFreeData =
      ValueNotifier(const AsyncData());
  Future<void> submitServiceFree(FormGroup formGroup) async {
    try {
      serviceFreeData.value = const AsyncData(loading: true);
      final response = await saleRepository.postServiceFee(
        ServiceFeeRequest.fromJson(formGroup.control('serviceFee').value),
      );
      serviceFreeData.value = AsyncData(data: response);
  ...
  ValueNotifier<AsyncData<ExpensesResponse>> expensesData =
      ValueNotifier(const AsyncData());
  ...
  void insertExpenses(FormGroup formGroup, ExpensesResponse? data) {
    formGroup.control('majorItems').value = data?.majorItems;
    ...
    formGroup.control('unitPrice').value = data.unitPrice;
    formGroup.control('amountOfMoney').value = data.amountOfMoney;
    formGroup.control('paymentDocument').value = data.paymentDocument;
    formGroup.control('totalExpenses').value = data.totalExpenses;
    formGroup.control('totalExpensesTax').value = data.totalExpensesTax;
    formGroup.control('totalExpensesAmount').value = data.totalExpensesAmount;
    formGroup.control('typeOfTax').value = data.typeOfTax;
  }

  ValueNotifier<AsyncData<ExpensesResponse>> submitExpensesData =
      ValueNotifier(const AsyncData());
  ...
  ValueNotifier<AsyncData<TotalResponse>> totalData =
      ValueNotifier(const AsyncData());
  ...
  void insertTotal(FormGroup formGroup, TotalResponse? data) {
    formGroup.control('totalSales').value = data?.totalSales;
    formGroup.control('totalSaleTax').value = data?.totalSaleTax;
    formGroup.control('totalCost').value = data?.totalCost;
    formGroup.control('totalCostTax').value = data?.totalCostTax;
    formGroup.control('grossProfit').value = data?.grossProfit;
    formGroup.control('grossProfitTax').value = data?.grossProfitTax;
    formGroup.control('taxIncluded').value = data?.taxIncluded;
    formGroup.control('taxExcluded').value = data?.taxExcluded;
    formGroup.control('taxExempt').value = data?.taxExempt;
    formGroup.control('grossProfitTax2').value = data?.grossProfitTax2;
  }

  ValueNotifier<AsyncData<TotalResponse>> submitTotalData =
      ValueNotifier(const AsyncData());
